(function () {
  ...
  async function init() {
    patchStorage();
    const cfg = window.GB_SUPABASE_CONFIG || {};
    if (!cfg.url || !cfg.anonKey || cfg.url.includes('PASTE_')) { updateStatusDot(false); return; }
    if (!window.supabase?.createClient) { updateStatusDot(false); scheduleInitRetry(); return; }
    ...
    if (pulled) {
      ...
      // First time or offline start: keep local data, try to push when possible.
      await push(true);
      updateStatusDot(navigator.onLine !== false);
    }
    subscribeRealtime();
